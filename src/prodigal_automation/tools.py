...
class ContentRequest(BaseModel):
    ...
    topic: str = Field(
        ..., min_length=2, max_length=100, description="Topic of the tweet"
    )
    length: int = Field(280, ge=50, le=280, description="Maximum tweet length")
...
class ContentGenerator:
    ...
    def simple_generate(self, topic: str, length: int = 500) -> str:
        ...
        Returns:
            Generated tweet content
        """
        # Estimate character length based on
        # word count (approx 6 chars per word)
