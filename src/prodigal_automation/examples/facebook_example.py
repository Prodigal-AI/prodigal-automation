...
def main():
    ...
    image_url = input(
        "Enter an image URL to post (e.g., https://example.com/image.jpg): "
    )
    if image_url:
        print("Generating image post content...")
        # You might want to get a separate caption for the image
        image_caption = fb_automation.content_generator.generate_simple_content(
            f"short caption for an image about {image_topic}"
        )
    ...
    video_url = input(
        "Enter a video URL to post (e.g., https://example.com/video.mp4): "
    )
    if video_url:
        print("Generating video post content...")
        video_caption = fb_automation.content_generator.generate_simple_content(
            f"short caption for a video about {video_topic}"
        )
        video_post_result = fb_automation.facebook_manager.post_video(
            video_caption, video_url
        )
    ...
    if schedule_choice == "yes":
        future_time_str = input(
            "Enter scheduling time (YYYY-MM-DD HH:MM:SS, e.g., 2025-06-25 10:00:00): "
        )
        try:
            dt_object = datetime.datetime.strptime(future_time_str, "%Y-%m-%d %H:%M:%S")
            """
            Ensure the scheduled time is in the future and within Facebook's limits (10 mins to 30 days) # noqa 501
            """
            ...
            if scheduled_timestamp < current_timestamp + 600:  # 10 minutes
            ...
            elif scheduled_timestamp > current_timestamp + (
                30 * 24 * 60 * 60
            ):  # 30 days
                ...
                scheduled_post_result = fb_automation.facebook_manager.create_post(
                    scheduled_post_topic, scheduled_publish_time=scheduled_timestamp
                )
    ...
    post_id_for_insights = input(
        "Enter a Facebook Post ID to fetch insights (leave blank to skip): "
    )
    if post_id_for_insights:
        ...
        post_insights_result = fb_automation.facebook_manager.get_post_metrics(
            post_id=post_id_for_insights, metrics=metrics_to_fetch
        )
