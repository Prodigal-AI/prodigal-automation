...
class TestFacebookManager(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_auth = FacebookAuth(
            access_token=self.mock_access_token, page_id=self.mock_page_id
        )
        ...
        self.mock_content_generator_instance = (
            self.mock_content_generator_class.return_value
        )

        # Configure mock_client_instance
        # if it has attributes/methods used by FacebookManager's init
        self.mock_client_instance.auth = (
            self.mock_auth
        )  # Ensure auth property is available
        self.mock_client_instance.page_id = (
            self.mock_page_id
        )  # Ensure page_id is available
        self.mock_client_instance.initialize.return_value = (
            self.mock_client_instance
        )  # For chainable .initialize()
    ...
    def test_init_production_mode(self, MockContentGenerator, MockFacebookClient):
        ...
        self.assertEqual(
            manager.content_generator, self.mock_content_generator_instance
        )
    ...
    def test_create_post_success(self, MockContentGenerator, MockFacebookClient):
        ...
        self.mock_content_generator_instance.generate_simple_content.return_value = (
        ...
        self.mock_content_generator_instance.generate_simple_content.assert_called_once_with(  # noqa
            topic
        )
    ...
    def test_create_post_api_error(self, MockContentGenerator, MockFacebookClient):
        ...
        import facebook  # Need to import facebook here to get GraphAPIError # noqa

        self.mock_client_instance.put_object.side_effect = facebook.GraphAPIError(
            "Mock FB API Error"
        )
    ...
    def test_create_post_no_page_id(self, MockContentGenerator, MockFacebookClient):
        ...
        auth_no_page_id = FacebookAuth(
            access_token=self.mock_access_token, page_id=None
        )
        MockFacebookClient.return_value.auth = (
            auth_no_page_id  # Ensure mock client uses this auth
        )
        MockFacebookClient.return_value.page_id = (
            None  # Explicitly set page_id on mock client to None
        )
        ...
        self.mock_client_instance.put_object.assert_not_called()

    @patch("prodigal_automation.facebook_manager.FacebookClient")
    @patch("prodigal_automation.facebook_manager.ContentGenerator")
    def test_create_post_scheduled_success(
        self, MockContentGenerator, MockFacebookClient
    ):
        ...
        self.mock_content_generator_instance.generate_simple_content.return_value = (
            "Scheduled test content."
        )
        self.mock_client_instance.put_object.return_value = {
            "id": "scheduled_post_id_456"
        }
    ...
    def test_get_page_metrics_success(self, MockContentGenerator, MockFacebookClient):
        ...
        expected_insights = {
            "data": [{"name": "page_impressions", "values": [{"value": 500}]}]
        }
    ...
    def test_delete_post_success(self, MockContentGenerator, MockFacebookClient):
        ...
        self.mock_client_instance.delete_object.return_value = {
            "success": True,
